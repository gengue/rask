...
export function AppShell(): JSX.Element {
   ...
   *
   * `?signin=` only ever arrives from the OAuth callback, which redirects here
   * rather than answering with a page of plain text nobody can navigate out of.
   */
  const signInRefusal = () => (search() as { signin?: string }).signin;
  if (signInRefusal()) markSignedOut();
  ...
  /*
   * Nothing of the workspace renders without a session.
   *
   * A `<Show>` and not `if (signedOut()) return …`: a Solid component body runs
   * once, so the early return would be evaluated at mount and never again, and
   * signing out would leave the workspace on screen with a dead session behind
   * it. This unmounts the whole subtree, which is also what stops SSE and the
   * pollers.
   *
   * Ahead of the router rather than as a `/login` route: every route below
   * needs the session anyway, and a route would have to be carved out of the
   * catch-all that translates ClickUp URLs.
   */
  return (
    <Show when={!signedOut()} fallback={<Login reason={signInError(signInRefusal())} />}>
